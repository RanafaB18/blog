...
const EditPost = ({ title, excerpt, id }: IEdit) => {
  const data = useContext(DataContext);
  if (!data) return <></>;
  const { setPosts } = data;
  function handleDelete() {
    setPosts((prevState) => {
      const updatedPosts = prevState.filter((post) => post.id !== id);
  ...
  return (
    <motion.div
      layout
      className="flex flex-col gap-4 border p-3 rounded-lg shadow-lg w-full"
    >
      <p>{title}</p>
      <p>{excerpt}</p>
